...
- git clone - копирует проект на локальный компьютер, автоматически связывает локальный репозиторий с удалённым.  

---  

# Действия с ветками

- git branch - можно посмотреть, какие в проекте есть ветки и в какой из них вы сейчас находитесь.  
- git branch -a - показать все ветки(на пк и в GitHub)  
- git branch -D - удаляет ветку на локальном пк.
- git branch -d - щадящее удаление на локальном пк(удалит ветку только если она была полностью объединена с другой)
- git checkout __название ветки__ - переключение между ветками.  
- git checkout -b __название ветки__ - создать и переключится на ветку.  

# СЛИЯНИЕ ВЕТОК  

- git merge __название ветки__ - Сливает указанную ветку в текущую.  
- По умолчанию используется fast-forward, его можно отключить с помощью флага --no-ff

# Забрать изменения из GitHub  

- git pull - позволяет "подтянуть" изменения из удаленного репозитория.  
...
- rebase - можно решить проблему «разошедшихся» веток с помощью операции rebase. Эта операция позволяет изменить точку (коммит), от которой отделилась ветка.  
git push --force - гит удалит коммиты, мешающие fast-forward с помощью пуша и запушит.  
